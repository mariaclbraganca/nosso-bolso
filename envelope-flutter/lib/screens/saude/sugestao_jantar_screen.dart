  ...
  Widget _buildHeader() {
    return const Padding(
      ...
      child: Column(
        ...
        children: [
          Text('Sugestão de Jantar', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: AppColors.tx)),
          SizedBox(height: 4),
          Text('Baseada no seu estoque e metas do dia', style: TextStyle(fontSize: 13, color: AppColors.mu)),
        ],
      ),
    );
  }
  ...
  Widget _buildItemsSuspeitos() {
    ...
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('Verificação Rápida', style: TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: AppColors.org)),
          const SizedBox(height: 8),
          ...suspeitos.map((item) => Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Expanded(child: Text('${item['nome']} (${item['dias_na_geladeira']}d)', style: const TextStyle(color: AppColors.tx, fontSize: 12))),
              Row(children: [
                TextButton(
                  onPressed: () {},
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text('Erro ao buscar sugestões', style: const TextStyle(color: AppColors.tx, fontSize: 16)),
            const SizedBox(height: 8),
            Text(_erro ?? '', style: const TextStyle(color: AppColors.mu, fontSize: 12), textAlign: TextAlign.center),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 16),
            Text('Sem estoque para sugestões', style: TextStyle(color: AppColors.tx, fontSize: 16)),
            SizedBox(height: 8),
            Text('Registre compras para que o agente saiba o que há na sua despensa.', style: TextStyle(color: AppColors.mu), textAlign: TextAlign.center),
          ],
        ),
      ),
    );
...
class _SugestaoCard extends StatelessWidget {
  ...
  const _SugestaoCard({required this.sugestao, required this.index, required this.onRegistrar});
      ...
      child: Column(
        ...
        children: [
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      sugestao['nome'] as String? ?? '',
                      style: const TextStyle(fontSize: 15, fontWeight: FontWeight.bold, color: AppColors.tx),
                    ),
                    if (preparo > 0)
                      Text('⏱ $preparo min de preparo', style: const TextStyle(fontSize: 11, color: AppColors.mu)),
                  ],
                ),
              ),
            ],
          ...
          if (sugestao['modo_preparo_resumido'] != null) ...[
            ...
            Text(
              sugestao['modo_preparo_resumido'] as String,
          ...
          if (substituicoes.isNotEmpty) ...[
            ...
            Container(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  const Text('Substituições:', style: TextStyle(fontSize: 10, color: AppColors.mu, fontWeight: FontWeight.bold)),
                  ...substituicoes.map((s) => Text('• $s', style: const TextStyle(fontSize: 11, color: AppColors.mu))),
                ],
              ),
            ),
          ],
          ...
          SizedBox(
            ...
            child: OutlinedButton(
              ...
              child: const Text('Registrar esta refeição', style: TextStyle(fontWeight: FontWeight.w600)),
            ),
          ),
        ],
      ),
