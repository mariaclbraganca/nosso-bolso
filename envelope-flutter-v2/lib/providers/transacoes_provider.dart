...
final transacoesComDetalhesProvider = Provider<List<Map<String, dynamic>>>((ref) {
  ...
  return transacoes.where((t) {
    ...
    if (!isAdmin && t['visivel_apenas_admin'] == true) return false;
...
class MesStats {
  ...
  final double totalAbastecido;
  final double saldo;
  MesStats({this.totalReceita = 0, this.totalDespesa = 0, this.totalAbastecido = 0, this.saldo = 0});
}
...
final statsPorMesProvider = Provider.family<MesStats, String>((ref, mes) {
  ...
  for (var t in transacoes) {
    ...
    if (data.startsWith(mes)) {
      ...
      if (t['tipo'] == 'abastecimento') abast += val;
    }
  }

  return MesStats(totalReceita: rec, totalDespesa: desp, totalAbastecido: abast, saldo: rec - desp);
});
